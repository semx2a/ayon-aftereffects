...
class ProcessLauncher(QtCore.QObject):
    ...
    def __init__(self, subprocess_args, host=None):
        ...
        # Held rather than looked up in 'registered_host()' on exit: the
        #   registry can still be empty when 'exit()' runs early, and a
        #   lock that is not released then is a lock nobody can clear.
        self._host = host
        self._log = None
